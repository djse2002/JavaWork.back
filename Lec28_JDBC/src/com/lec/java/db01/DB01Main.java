...
//			sql_insert = "INSERT INTO test_member VALUES(" + no + ", '" + name + "', SYSDATE)";
			...
			rs = stmt.executeQuery(sql_select); // 'SELECT' 및 기타 쿼리의 경우 executeQuerty() 로 실행
										   // 리턴값은 ResultSet 객체
			...
			// 7-1 컬럼 이름으로 받기
//			System.out.println();
//			while(rs.next()) { // next() 레코드 하나 추출하고 True 리턴, 
...
//			while (rs.next()) { // next() 레코드 하나 추출하고 True 리턴,
//				// 더이상 뽑아낼 레코드 없으면 false 리턴
//				String no = rs.getString(1); // getXXX() 에 '인덱스 명시
				...
				// java.sql.Date , java.sql.Time
				 ...
				 if(d != null) {
					 birthdate = new SimpleDateFormat("yyyy년MM월dd일").format(d) + " " 
							 + new SimpleDateFormat("hh:mmLss").format(t);
				 }
