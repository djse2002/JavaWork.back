...
public class Collection14Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		Collections.sort(list3);     // Student 에 Comparable<> 구현 되지 있지 않으면  에러!
		// Comparable<> 구현
		...
		Collections.sort(list3, new Desc()); // 이름 오름차순으로 정렬
		System.out.println(list3);
		...
		List<String> arrList = Arrays.asList(arr); // 배열을  --> List<> 변환 (List<> 의 toArray() 의 반대)
		Collections.shuffle(arrList); 
		arrList = arrList.subList(0, 3); // index 0 부터 3 전까지 List<> 생성
...
class Student implements Comparable<Student>{
	...
	public int compareTo(Student o) {
		...
		// 점수 오름차순
//		if(o.point > this.point) return -1;   // 내가  더 작으면 -1
//		if(this.point > o.point) return 1;    // 내가 더 크면 1
//		return 0; // 같으면 0
...
// Student 를 이름(name) 으로 정렬해줄수 있는 comparator<> 제공
