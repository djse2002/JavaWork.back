...
public class File16Main {
	...
	public static void main(String[] args) {
		...
		//File 객체 생성.  ---> 디스크에 물리적인 파일 / 디렉토리가 생성되는 것은 아니다.!!!!
		...
		if(f2.exists()) { // "test/dummy.txt" 존재하는지 체크
			// 변경
			if(f2.renameTo(f3)) {
				System.out.println("파일 이름 변경 성공!");
			}else {
				System.out.println("파일 이름 변경 실패");  //이미 "re_dummy.tet"가 존재하면 실패 한다.
			}
		...
		// 파일 삭제: delete()
		// TODO
		File f4 = new File(f, TEST_RENAME);  // "redummy.txt"
		...
		if(f4.exists()) {
			...
			System.out.println("삭제 할 파일이 없습니다.");
		}
